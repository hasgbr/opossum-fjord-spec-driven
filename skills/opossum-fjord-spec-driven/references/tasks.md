...
**Mirrors:** the generated matrix (Test Coverage Matrix + Gate Check Commands) is the source for the `.specs/TEST-RESULTS.md` mirror (test outcomes) and feeds the live `.specs/CHECKLIST-MANUAL.md` (manual-validation checklist). When tasks are approved and executed, keep these mirrors updated in the same commits as the work - never defer them. See [repo-docs.md](repo-docs.md).
...
Implement these tasks with the `opossum-fjord-spec-driven` skill: **activate it by name and follow its Execute flow and Critical Rules.** Do not search for skill files by filesystem path. The skill is the source of truth for the full flow (per-task cycle, sub-agent delegation, adequacy review, Verifier, discrimination sensor, checkpoints, .specs mirrors).
