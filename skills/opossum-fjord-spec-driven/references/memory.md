...
**Project-kickoff decisions are always recorded.** On the first interaction with a project, the two methodology decisions are fixed as AD entries and honored for the whole project unless superseded (see SKILL.md):

- **Checkpoints on/off** — the ~2 sub-step cadence is recommended but decided with the user; when on, Execute pauses at each checkpoint. See [checkpoints.md](checkpoints.md).
- **Frontend-first on/off** — whether to ask per user-facing feature for the FE mock → validate → BE real cycle. See [frontend-first.md](frontend-first.md).

These are project-level by definition (every feature must follow them), so they earn an AD-NNN entry on kickoff.
