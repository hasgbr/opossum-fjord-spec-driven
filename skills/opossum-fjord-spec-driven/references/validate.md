...
**Frontend-first mode:** in a project running frontend-first (see [frontend-first.md](frontend-first.md)), a user-facing feature is validated in the browser **twice** — first the FE mock (before the real backend is wired), then the real integration. The FE-mock validation is a hard gate before backend work: the user must approve the UI/interaction in the browser before the backend is built. Both validations are recorded in `.specs/CHECKLIST-MANUAL.md` (the live manual checklist). See [repo-docs.md](repo-docs.md).
