...
**Two methodology extensions apply inside Execute** (see the skill's SKILL.md):

- **Checkpoints** — if the project has checkpoints on (recorded in `.specs/STATE.md`), pause and return control to the user every ~2 sub-steps; never chain sub-step after sub-step without a checkpoint. See [checkpoints.md](checkpoints.md).
- **.specs documentation mirrors** — at the end of each phase (and of the feature), update `.specs/CHANGELOG.md`, `.specs/TEST-RESULTS.md`, `.specs/CHECKLIST-MANUAL.md`, and add ADRs for decisions, in the same commit as the phase. See [repo-docs.md](repo-docs.md).
...
### Checkpoints (if the project has them on)

Check `.specs/STATE.md` for the checkpoints decision (AD-NNN). If checkpoints are **on**, implement at most ~2 small sub-steps, then **STOP and return control to the user** for validation before continuing - never chain sub-step after sub-step without a checkpoint. A single large sub-step that aggregates many design items already satisfies a checkpoint. Resume only after the user reports validation (or explicitly asks to continue). See [checkpoints.md](checkpoints.md).
...
**Mirrors in the commit.** When this task closes a phase (or the feature), the same commit must include the `.specs/` documentation-mirror updates: `CHANGELOG.md`, `TEST-RESULTS.md`, `CHECKLIST-MANUAL.md`, and any ADR. These are not optional and are never deferred to a later commit. See [repo-docs.md](repo-docs.md).

**Push follows authorization (not commit).** A commit that exists only locally does NOT count as done. `git push` requires an explicit go-ahead (see Blast radius in step 8), but once the user authorizes a push, push the commit immediately - do not leave it local-only waiting. When the user authorizes a phase, that does not by itself authorize a push; ask for the push go-ahead, and on approval push right away.
...
**Frontend-first UAT:** if the feature is user-facing and the project runs in frontend-first mode (see [frontend-first.md](frontend-first.md)), the Execute cycle includes validating the FE mock in the browser with the user BEFORE wiring the real backend. The Verifier's interactive UAT step (step 9) covers this.

**Mirrors before done:** a feature is not "done" until the `.specs/` mirrors (CHANGELOG, TEST-RESULTS, CHECKLIST-MANUAL, ADRs) reflect it. See [repo-docs.md](repo-docs.md).
