...
Merged skill extension: when a feature's report passes, a SOFT audit of the
.specs/ documentation mirrors (CHANGELOG.md, TEST-RESULTS.md,
CHECKLIST-MANUAL.md) runs and reports warnings (does not change the exit code).
The completion gate semantics are preserved; the mirror audit is advisory so a
backend-only feature with no test-mirror updates is not falsely blocked.
...
def _mirror_audit(root):
    """SOFT advisory: confirm the .specs/ documentation mirrors are present and
    non-trivial. Returns a list of warning strings (never fails the gate)."""
    warnings = []
    specs = os.path.join(root, ".specs")
    if not os.path.isdir(specs):
        return warnings
    for name, label in (("CHANGELOG.md", "CHANGELOG"), ("TEST-RESULTS.md", "TEST-RESULTS"),
                        ("CHECKLIST-MANUAL.md", "CHECKLIST-MANUAL")):
        path = os.path.join(specs, name)
        if not os.path.isfile(path):
            warnings.append(f"mirror: .specs/{name} ({label}) missing - see repo-docs.md")
            continue
        body = open(path, encoding="utf-8", errors="replace").read()
        if len(body.strip()) < 40:
            warnings.append(f"mirror: .specs/{name} ({label}) is empty/placeholder - see repo-docs.md")
    return warnings
